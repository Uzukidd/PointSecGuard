...
class DistortionBenchmark(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        init_kwargs["samples_batch_size"] = (
            batch_size  # use same batch_size for nes, spsa, nattack's samples
        )
        ...
        self.batch_size, self.goal, self.distance_metric = (
            batch_size,
            goal,
            distance_metric,
        )
        ...
        self._xs_ph = tf.placeholder(
            self.model.x_dtype, shape=(None, *self.model.x_shape)
        )
    ...
    def _run_binsearch(self, dataset, logger):
        ...
        for i_batch, (_, xs, ys, ys_target) in enumerate(iterator):
            ...
            ys_range = np.arange(
                0, self.batch_size * self.model.n_class, self.model.n_class
            )
            ys_flatten = ys_range.astype(self.model.y_dtype.as_numpy_dtype) + ys
            ys_target_flatten = (
                ys_range.astype(self.model.y_dtype.as_numpy_dtype) + ys_target
            )
            del ys_range
            ...
            for i in range(2**self.search_steps):
                ...
                logits = self._session.run(
                    self._logits, feed_dict={self._xs_ph: xs_adv}
                )
                ...
                if self.goal == "ut" or self.goal == "tm":
                    ...
                    # adversarial.
                    succ = (
                        logits.max(axis=1) - logits.take(ys_flatten) > self.confidence
                    )
                else:
                    ...
                    logits_that = np.nanmax(
                        logits.reshape((self.batch_size, -1)), axis=1
                    )
                ...
                if logger:
                    ...
                    logger.info(
                        "linsearch n={}..{}: i={}, success_rate={:.3f}".format(
                            begin, begin + len(xs) - 1, i, succ.astype(float).mean()
                        )
                    )
                if np.all(succ):
                    break
        ...
        for i_batch, (_, xs, ys, ys_target) in enumerate(iterator):
            ...
            for i in range(self.search_steps):
                ...
                if logger:
                    begin = i_batch * len(xs)
                    logger.info(
                        "search n={}..{}: i={}, success_rate={:.3f}".format(
                            begin, begin + len(xs) - 1, i, succ.astype(float).mean()
                        )
                    )
                if np.all(succ):
        ...
        for n, (_, x, y, y_target) in enumerate(iterator):
            ...
            for i in range(self.search_steps):
                if self.attack_name == "nes":
                    self.attack.config(
                        magnitude=hi,
                        lr=hi * self.nes_lr_factor,
                        min_lr=hi * self.nes_min_lr_factor,
                    )
            ...
            for i in range(self.binsearch_steps):
                ...
                if self.attack_name == "nes":
                    self.attack.config(
                        magnitude=mi,
                        lr=mi * self.nes_lr_factor,
                        min_lr=mi * self.nes_min_lr_factor,
                    )
                elif self.attack_name == "spsa":
