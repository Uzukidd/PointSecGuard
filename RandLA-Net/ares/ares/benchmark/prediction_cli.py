...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "--count", help="Number of examples to predict.", type=int, required=True
    )
    PARSER.add_argument(
        "--output", help="Path to save prediction result.", type=str, required=True
    )
    ...
    PARSER.add_argument(
        "--batch-size",
        help="Batch size for running prediction.",
        type=int,
        required=True,
    )
    ...
    for i_batch, (_, xs, ys, ys_target) in enumerate(
        ...
        logger.info(
            "n={}..{} acc={:3f}".format(
                i_batch * batch_size, i_batch * batch_size + batch_size - 1, acc
            )
        )
    ...
    logger.info(
        "acc={:3f}".format((rs["predictions"] == rs["ys"]).astype(float).mean())
    )
