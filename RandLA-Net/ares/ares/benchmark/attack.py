...
from ares.benchmark.utils import gen_starting_points, load_attack
...
class AttackBenchmark(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.batch_size, self.goal, self.distance_metric, self.session = (
            batch_size,
            goal,
            distance_metric,
            session,
    ...
    def run(self, dataset, logger):
        ...
        def update(accs, accs_adv, totals, succs, dists):
            ...
            if logger is not None:
                logger.info(
                    "acc={:3f}, adv_acc={:3f}, succ={:3f}, dist_mean={:3f}".format(
                        np.mean(accs.astype(float)),
                        np.mean(accs_adv.astype(float)),
                        np.sum(succs.astype(float)) / np.sum(totals.astype(float)),
                        np.mean(dists),
                    )
                )
        ...
        if self.attack_name in ("fgsm", "bim", "pgd", "mim", "cw", "deepfool"):
            ...
            for _, xs, ys, ts in iterator:
                ...
                xs_adv_pred = self.session.run(
                    self.xs_label, feed_dict={self.xs_ph: xs_adv}
                )
            ...
            for _, x, y, t in iterator:
                ...
                x_adv_pred = self.session.run(
                    self.xs_label, feed_dict={self.xs_ph: [x_adv]}
                )[0]
                xs, xs_adv, ys, ts = (
                    np.array([x]),
                    np.array([x_adv]),
