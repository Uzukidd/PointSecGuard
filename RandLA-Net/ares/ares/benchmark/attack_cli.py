...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "--count", help="Number of examples to attack.", type=int, required=True
    )
    ...
    PARSER.add_argument(
        "--goal", help="Attack goal.", required=True, choices=["t", "tm", "ut"]
    )
    PARSER.add_argument(
        "--distance-metric",
    ...
    PARSER.add_argument(
        "--batch-size",
        help="Batch size hint for attack method.",
        type=int,
        required=True,
    )
    PARSER.add_argument(
        "--learning-rate", help="Learning rate in CW attack.", type=float
    )
    PARSER.add_argument(
        "--cw-loss-c", help="CWLoss's c value in CW attack.", type=float
    )
    ...
    if args.dataset == "cifar10":
        ...
        dataset = cifar10.load_dataset_for_classifier(
            model, offset=args.offset, load_target=True
        )
    else:
    ...
    for kwarg in ("learning_rate", "cw_loss_c", "samples_per_draw", "init_distortion"):
        ...
        if attr is not None:
            kwargs[kwarg] = attr
    if (
        args.dimension_reduction_height is not None
        and args.dimension_reduction_width is not None
    ):
        kwargs["dimension_reduction"] = (
            args.dimension_reduction_height,
    ...
    print(
        "n={}, acc={:3f}, adv_acc={:3f}, succ={:3f}, dist_mean={:3f}".format(
            args.count,
            np.mean(acc.astype(float)),
            np.mean(acc_adv.astype(float)),
            np.sum(succ.astype(float)) / np.sum(total.astype(float)),
            np.mean(dist),
        )
    )
