...
class NUattack(BatchAttack):
    def __init__(
        ...
        xs_shape_flatten = (
            model.config.val_batch_size,
            np.prod((model.config.num_points, 3)),
        )
        num_layers = model.config.num_layers

        self.xs_ph = tf.placeholder(tf.float32, self.feature_shape)
        self.ys_ph = tf.placeholder(
            tf.int32, (model.config.val_batch_size, model.config.num_points)
        )
        ...
        ys_var = tf.Variable(
            tf.zeros(
                shape=(model.config.val_batch_size, model.config.num_points),
                dtype=tf.int32,
            )
        )
        self.ys_var = ys_var
        ...
        self.xs_adv_model = tf.reshape(
            self.xs_adv, (model.config.val_batch_size, model.config.num_points, 3)
        )
        self.other_features_ph = tf.placeholder(tf.float32, self.feature_shape)
        self.features_ph = tf.concat(
            (self.other_features_ph, self.xs_adv_model), axis=2
        )
        # the C&W loss term
        ...
        ge.connect(
            ge.sgv(self.features_ph),
            ge.sgv(model.inputs_features),
        ...
        self.setup_optimizer = tf.compat.v1.variables_initializer(
            adv_optimizer.variables()
        )
    ...
    def config(self, **kwargs):
        if "cs" in kwargs:
            self.cs = maybe_to_array(
                kwargs["cs"], target_len=kwargs["model"].config.val_batch_size
            ).astype(np.float32)
    ...
    def NUloss(self, model, ys):
        ...
        real_logit = tf.reduce_sum(
            logits_mask * self.adv_logits, axis=-1
        )  # ys target label goal
        other_logit = tf.reduce_max(
            (1 - logits_mask) * self.adv_logits, axis=-1
        )  # adv goal
    ...
    def batch_attack(self, model, data_batch, session=None):
        ...
        ys_input = (
            ys_target if self.goal == "t" or self.goal == "tm" else labels
        ...
        ori_score, ori_logits, ori_dists = self._session.run(
            (self.score, model.logits, self.dists),
            feed_dict={
                ...
                model.inputs_input_inds: input_inds,
                model.inputs_cloud_inds: cloud_inds,
            },
        )
        ...
        for search_step in range(self.search_steps):
            ...
            for iii in range(self.iteration):
                ...
                new_score, new_logits, new_xs_adv, new_dists, inds, cloud_inds = (
                    self._session.run(
                        (
                            self.score,
                            self.adv_logits,
                            self.xs_adv_model,
                            self.dists,
                            model.inputs_input_inds,
                            model.inputs_cloud_inds,
                        ),
                        feed_dict={
                            model.inputs_xyz: xyz,
                            model.is_training: False,
                ...
                if self.goal == "ut" or self.goal == "tm":
                    ...
                    # adversarial.
                    new_succ = (
                        new_logits.max(axis=1) - new_logits.take(ys_flatten)
                    ) > self.confidence
                else:
                ...
                if acc < 1 / 13:
                    ...
                    rand_score, rand_logits, rand_dists = self._session.run(
                        (self.score, model.logits, self.dists),
                        feed_dict={
                            model.inputs_xyz: xyz,
                            model.is_training: False,
                    ...
                    self.logger.info(
                        "cloud={}, acc={}, original_acc={}, new_dists={}, mIoU={}, original_mIoU={}, rand_acc={}, rand_mIoU={}".format(
                            cloud_inds,
                            acc,
                            original_accuracy,
                            new_dists,
        ...
        for binsearch_step in range(self.binsearch_steps):
            ...
            for iii in range(self.iteration):
                self._session.run(
                    ...
                    feed_dict={
                        ...
                        self.xyz_ph4: xyz[3],
                        self.xyz_ph5: xyz[4],
                        self.neigh_idx1: neigh_idx[0],
                        self.neigh_idx2: neigh_idx[1],
                        self.neigh_idx3: neigh_idx[2],
                        self.neigh_idx4: neigh_idx[3],
                        self.neigh_idx5: neigh_idx[4],
                        self.sub_idx1: sub_idx[0],
                        self.sub_idx2: sub_idx[1],
                        self.sub_idx3: sub_idx[2],
                        self.sub_idx4: sub_idx[3],
                        self.sub_idx5: sub_idx[4],
                        self.interp_idx1: interp_idx[0],
                        self.interp_idx2: interp_idx[1],
                        self.interp_idx3: interp_idx[2],
                        self.interp_idx4: interp_idx[3],
                        self.interp_idx5: interp_idx[4],
                        self.other_features_ph: features[:, :, :3],
                        self.ys_ph: labels,
                        self.input_inds_ph: input_inds,
                        self.cloud_inds_ph: cloud_inds,
                        model.is_training: False,
                    },
                )

                new_score, new_logits, new_xs_adv, new_dists = self._session.run(
                    (self.score, self.adv_logits, self.xs_adv_model, self.dists),
                    feed_dict={
                        self.xyz_ph1: xyz[0],
                        self.xyz_ph2: xyz[1],
                        self.xyz_ph3: xyz[2],
                ...
                xs_adv[to_update], min_dists[to_update] = (
                    new_xs_adv[to_update],
                    new_dists[to_update],
                )
                succ[to_update] = True
            ...
            print(
                (
                    "binsearch_step={}, cs_mean={}, success_rate={}".format(
                        binsearch_step, cs.mean(), new_succ.astype(float).mean()
                    )
                )
            )
            if self.logger:
                self.logger.info(
                    "binsearch_step={}, cs_mean={}, success_rate={}".format(
                        binsearch_step, cs.mean(), succ.astype(float).mean()
                    )
                )
