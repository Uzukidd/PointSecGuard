...
class ScannetDatasetWholeScene():
    ...
    def __init__(self, root, block_points=4096, split='test', test_area=5, stride=0.5, block_size=1.0, padding=0.001):
        ...
        for file in self.file_list:
            ...
            # print(self.scene_points_list[0].shape, self.semantic_labels_list[0].shape)
            # exit()
            coord_min, coord_max = np.amin(points, axis=0)[:3], np.amax(points, axis=0)[:3]
            self.room_coord_min.append(coord_min), self.room_coord_max.append(coord_max)
    ...
    def __getitem__(self, index):
        ...
        for index_y in range(0, grid_y):
            for index_x in range(0, grid_x):
                ...
                data_batch[:, 0] = data_batch[:, 0] - (s_x + self.block_size / 2.0) # change x
                data_batch[:, 1] = data_batch[:, 1] - (s_y + self.block_size / 2.0) # change y
...
if __name__ == '__main__':
    data_root = '/home/jxu/pc_attack/datasets/stanford_indoor3d/'
