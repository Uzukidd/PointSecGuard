...
class NB_attack(Attack):
    def __init__(
        self,
        model,
        coord_range: Union[np.ndarray, torch.Tensor],
        data_xy_offset: Union[np.ndarray, torch.Tensor],
        eps=0.3,
        alpha=2 / 255,
        iters=40,
        ...
        self.data_xy_offset = data_xy_offset
        if isinstance(self.data_xy_offset, np.ndarray):
            self.data_xy_offset = torch.from_numpy(self.data_xy_offset).to(self.device)
    ...
    def forward(self, images, labels):
        ...
        for i in range(self.iters):
            ...
            if self.use_coord:
                adv_images[:, :3] = coord
                adv_images[:, 6:] = self.normalize_pts(adv_images[:, :3].permute(0, 2, 1), self.data_xy_offset, self.coord_range).permute(0, 2, 1)

            outputs, _ = self.model(adv_images)

            self.model.zero_grad()
            cost = (
                loss(outputs.reshape(-1, outputs.size(2)), labels.view(-1))
                / outputs.size(1)
            ).to(self.device)
            cost.backward()
            ...
            if self.use_color:
                ...
                eta = torch.clamp(
                    adv_images[:, 3:6] - ori_image[:, 3:6], min=-self.eps, max=self.eps
                )
            ...
            if self.use_coord:
                ...
                eta = torch.clamp(
                    adv_images[:, :3] - ori_image[:, :3], min=-self.eps, max=self.eps
                )
...
class NU_attack(Attack):
    ...
    def forward(self, images, labels):
        ...
        optimizer = optim.Adam([coord], lr=self.lr)
        ...
        for step in range(self.steps):
            ...
            Smooth_loss = self.coord_smooth(adv_images, images, neighour).mean()
            cost = f_loss
            ...
            if acc < 1 / 13:
                ...
                if (step > 10) & (step % 10 == 0):
                    if cost.item() > prev_cost[step - 10]:
                        # print("======bingo======")
                        images[:, 3:6][:, :, mask] = images[:, 3:6][
                            :, :, mask
                        ] + torch.empty_like(images[:, 3:6][:, :, mask]).uniform_(0, 1)
    ...
    def f(self, outputs, labels):
        ...
        one_hot_labels = torch.eye(len(outputs[0]), device=outputs.device)[labels].to(
            self.device
        )
    ...
    def coord_smooth(self, adv_images, images, neighbour):
        """
        Args:
            adv_images: [B, 6, N]
            images: [B, 6, N]
        ...
        Returns:
            smooth_dist: [B, N, neighbour]
        """
        # 获取 [B, N, 3] 的坐标张量
        adv_pos = adv_images[:, :3].permute(0, 2, 1)  # [B, N, 3]
        pos = images[:, :3].permute(0, 2, 1)  # [B, N, 3]

        knn_result = knn_points(adv_pos, pos, K=neighbour)

        return knn_result.dists.sum((1, 2))
