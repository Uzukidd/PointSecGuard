...
def proj_surface(x, n):
    ...
    Args:
        ...
        n: [B, n, 3]
    """
    beta = (x * n).sum((1, 2)) / (n * n).sum((1, 2)).clamp(min = 1e-12)
    alpha = x - beta[:, None, None] * n
    return alpha


def gram_schmidt(g, delta, need_gradient:bool = False):
    """
    Args:
        g: [B, n, 3]
        delta: list of [B, n, 3]
...
class boundary_projection_seg(boundary_projectuion):
    ...
    def __init__(
        ...
        super(boundary_projection_seg, self).__init__(
            l2_weight,
            hd_weight,
            cd_weight,
            curv_weight,
    ...
    def reset(self, batch_size: int):
        ...
        self.output_losses = (
            torch.ones((batch_size, self.optims.__len__())).cuda() * 1e5
        )
        self.output_points = None
