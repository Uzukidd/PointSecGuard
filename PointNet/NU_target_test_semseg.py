...
def parse_args():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=1, help="batch size in testing [default: 32]"
    )
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/datasets/stanford_indoor3d/",
        help="path to the dataset",
    )
    parser.add_argument(
        "--log_dir",
        type=str,
        default="tar_adv_pointnet2_sem_seg",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="the left points ratio for the L0 attack",
    )
    parser.add_argument(
        "--origin",
        default=11,
        type=int,
        help="the target points of the original labels",
    )
    parser.add_argument(
        "--target", default=6, type=int, help="the traget label of the target points"
    )
...
def main(args):
    ...
    if True:
        ...
        with open(os.path.join(experiment_dir, "log_" + timestr + ".txt"), "w") as f:
            f.write(
                "ori\tindex\tL2_dis\tcount\t target acc\tadv_acc\tacc\tadv_miou\tmiou\n"
            )
        ...
        for batch_idx in range(num_batches):
            ...
            for _ in tqdm(range(args.num_votes), total=args.num_votes):
                ...
                )  # (676, 4096, 9) (676, 4096) (676, 4096) (676, 4096)
                ...
                for sbatch in range(s_batch_num):
                    ...
                    mask = np.in1d(batch_label[0:real_batch_size, ...], ori).reshape(
                        batch_label[0:real_batch_size, ...].shape
                    )
                    count = (mask == True).sum()
                    ...
                    if count == 0:
                        ...
                        attack = torchattacks.tar_NU_attack(
                            classifier,
                            c=1,
                            kappa=0,
                            steps=1000,
                        ...
                        adv_images = attack(
                            torch_data, batch_label[0:real_batch_size, ...]
                        )  # (batch, 6, 4096)

                    dis = torch.dist(adv_images, torch_data)
                    target_labels = torch.full(
                        batch_label[0:real_batch_size, ...].shape, target
                    ).cuda()
                    adv_seg_pred, _ = classifier(adv_images)

                    temp_ind = np.reshape(
                        batch_point_index[0:real_batch_size], (-1,)
                    ).astype(int)
                    adv_whole_scene[temp_ind] = np.reshape(
                    ...
                    adv_batch_pred_label = (
                        adv_seg_pred.contiguous().cpu().data.max(2)[1].numpy()
                    )
                    ...
                    if count != 0:
                        target_acc = (
                            tmp_adv_batch_pred_label.unsqueeze(0)[:, mask]
                            .eq(
                                target_labels.unsqueeze(0)[:, mask].view_as(
                                    tmp_adv_batch_pred_label.unsqueeze(0)[:, mask]
                                )
                            )
                            .sum()
                    ...
                    acc = pred.eq(gt.view_as(pred)).sum().item() / (
                        4096 * torch_data.shape[0]
                    )
                    adv_pred = adv_seg_pred.max(dim=2)[1]
                    adv_acc = adv_pred.eq(gt.view_as(adv_pred)).sum().item() / (
                        4096 * torch_data.shape[0]
                    )
                    ...
                    for l in range(NUM_CLASSES):
                        ...
                        single_total_correct_class_tmp[l] += np.sum(
                            (pred.detach().cpu().numpy() == l)
                            & (single_whole_scene_label == l)
                        )
                        single_total_iou_deno_class_tmp[l] += np.sum(
                            (
                                (pred.detach().cpu().numpy() == l)
                                | (single_whole_scene_label == l)
                            )
                        )
                        single_adv_total_correct_class_tmp[l] += np.sum(
                            (adv_pred.detach().cpu().numpy() == l)
                            & (single_whole_scene_label == l)
                        )
                        single_adv_total_iou_deno_class_tmp[l] += np.sum(
                            (
                                (adv_pred.detach().cpu().numpy() == l)
                                | (single_whole_scene_label == l)
                            )
                    ...
                    single_iou_map = np.array(single_total_correct_class_tmp) / (
                        np.array(single_total_iou_deno_class_tmp, dtype=float) + 1e-6
                    )
                    single_adv_iou_map = np.array(
                        single_adv_total_correct_class_tmp
                    ) / (
                        np.array(single_adv_total_iou_deno_class_tmp, dtype=float)
                        + 1e-6
                    )
                    ...
                    if count != 0:
                        ...
                        with open(
                            os.path.join(experiment_dir, "log_" + timestr + ".txt"),
                            "a+",
                        ) as f:
                            f.write(
                                "%d\t%d\t%.3f\t%d\t%.5f\t%.5f\t%.5f\t%.5f\t\t%.5f\n"
            ...
            for l in range(NUM_CLASSES):
                ...
                adv_total_correct_class_tmp[l] += np.sum(
                    (adv_pred_label == l) & (whole_scene_label == l)
                )
                adv_total_iou_deno_class_tmp[l] += np.sum(
                    ((adv_pred_label == l) | (whole_scene_label == l))
                )
            ...
            adv_iou_map = np.array(adv_total_correct_class_tmp) / (
                np.array(adv_total_iou_deno_class_tmp, dtype=float) + 1e-6
            )
            ...
            print(
                "pos distance: ",
                np.linalg.norm(whole_scene_data[:, :3] - adv_whole_scene[:, :3]),
            )
            print(
                "color distance: ",
                np.linalg.norm(whole_scene_data[:, 3:6] - adv_whole_scene[:, 3:6]),
            )
            ...
            log_string(
                "adv eval whole scene point avg class acc: %f"
                % (
                    np.mean(
            ...
            log_string(
                "adv eval whole scene point accuracy: %f"
                % (
                    np.sum(adv_total_correct_class)
